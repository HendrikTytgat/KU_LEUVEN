...
"""
__all__ = ["HinSAGE", "MeanHinAggregator", "MaxPoolingHinAggregator", "MaxHinAggregator"]
...
class MaxPoolingHinAggregator(HinSAGEAggregator):
    """
    Max Pooling Aggregator for HinSAGE implemented with Keras base layer

    Implements the aggregator of Eq. (3) in Hamilton et al. (2017) with changes to accomodate a heterogenous network.
    ...
    def group_aggregate(self, x_group, group_idx=0):
        ...
        if group_idx == 0:
            ...
            x_agg = K.dot(x_group, self.w_neigh[0])

        else:
            w_g, w_pool, b_pool = self.w_neigh[group_idx]
...
class MaxHinAggregator(HinSAGEAggregator):
    """Max Aggregator for HinSAGE implemented with Keras base layer
    ...
    def call(self, x, **kwargs):
        """
        Apply MaxAggregation on input tensors, x
        ...
        for r in range(self.nr):
            ...
            if z.shape[2] > 0:
                z_agg = K.dot(K.max(z, axis=2), self.w_neigh[r])
...
class HinSAGE:
    ...
    .. seealso::
       ...
       Aggregators: :class:`.MeanHinAggregator`, :class`.MaxHinAggregator`, :class`.MaxPoolingAggregator`,
    ...
    def __init__(
        ...
        if aggregator is None:
            self._aggregator = MaxPoolingHinAggregator
